...
This document describes how to write **behavioral** (ClawSec) YAML rules under `behavioral-templates/`. **Nuclei** rules use the standard Nuclei v3 format.

Implementation details live in **clawsec**: `internal/templates/schema.go`, `internal/templates/loader.go`, and `internal/detector/engine.go`.
